...
//                        .start();
	...
	private void loginSubmit() {
		mUsername = login_edit_yhm.getText().toString().trim();
		mPassword = login_edit_mm.getText().toString().trim();
		if (mUsername == null || mUsername.equals("")) {//帐号为空时
			showToast("请输入帐号");
			return;
		} else if (mPassword == null || mPassword.equals("")) {//密码为空时
			showToast("请输入密码");
			return;
		}
